...
import { CreateServiceTypeService } from "../services/CreateServiceTypeService";
...
serviceTypesRouter.get("/", async (request, response) => {
  ...
  try {
    ...
    return response
      .status(500)
      .json({ error: "Error processing the request for service types." });
  }
});
