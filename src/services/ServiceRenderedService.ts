...
class ServiceRenderedService {
  public async execute({
    ...
    const serviceRenderedRepository = AppDataSource.getRepository(ServiceRendered)
    ...
    if (serviceTypes) {
      ...
      if (!serviceTypeEntity) {
        throw new AppError(`Service type with name "${serviceTypes}" not found.`)
      }
      ...
      if (!serviceTypeEntity) {
        throw new AppError(`Service type with ID "${service_types_id}" not found.`)
      }
    } else {
    ...
    if (checkDuplicate) {
      throw new AppError('Military or service type already exists on the given date.')
    }
...
export default ServiceRenderedService
